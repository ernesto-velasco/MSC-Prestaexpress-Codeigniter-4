...
<script>
    pagoBaseLabel = document.querySelector('#pagoBase')
    montoInput = document.querySelector('input#monto')
    duracionInput = document.querySelector('input#duracion')

    calcularMontoBase()

    function calcularMontoBase() {
        pagoBaseLabel.innerText = (montoInput.value / duracionInput.value).toFixed(2)
    }

    montoInput.addEventListener('change', calcularMontoBase)
    duracionInput.addEventListener('change', calcularMontoBase)
</script>
<?= $this->endSection() ?>