...
class PrestamoController extends BaseController
{
    ...
    public function solicitud()
    {
        ...
        if (session('motivos')) return view('prestamo/solicitudDenegada');
    ...
    public function eliminar($id)
    {
        ...
        session()->setFlashdata('success', 'Préstamo eliminado');
        return redirect()->to(base_url('prestamos'));
    }
    ...
    public function detalles($id)
    {
        ...
        $data['prestamo'] = $prestamoModel
            ...
            ->join('empleado', 'empleado.id_empleado = prestamo.id_empleado')
            ->find($id);
        $data['abonos'] = $abonoModel->where('id_prestamo', $id)->find();
        $totalAbonoCapital = $abonoModel->selectSum('monto_capital')->where('id_prestamo', $id)->first();
